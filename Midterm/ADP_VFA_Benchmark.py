...
# INITIALIZATION FUNCTIONS:
    def initialize_state(self):
        ...
        aggregate_state_key = self.create_aggregate_state_key(state_dict)
...
######### HELPER FUNCTIONS: #########
    ...
    def create_aggregate_state_key(self, state_dict):
        """
...
################### SOLVE: ###################
    def train_with_vfa(self):
        ...
        for n in range(1, int(self.N)+1):
            ...
            for t in self.steps[:-1]:
                ...
                for aircraft_id in self.aircraft_ids:
                    ...
                    for a in ac_actions:
                        ...
                        temp_post_decision_state = self.simulate_action_to_state(current_state, a, t, n)
                        temp_post_decision_state_key = self.create_hashable_state_key(temp_post_decision_state)
                        temp_aggregate_state_key = self.create_aggregate_state_key(temp_post_decision_state)

                        R_t = self.compute_reward(current_state, temp_post_decision_state, a)

                        # Check if value of post_decision state value is known:
                        if temp_post_decision_state_key in self.states:
                            V_n = temp_post_decision_state['value'][-1]

                        # Check if value of aggregated post decision state is known:
                        elif temp_aggregate_state_key in self.agg_states:
                            # smooth the values of aggregated state and disaggregated state
                            v_agg = self.agg_states[temp_aggregate_state_key]['value'][-1]
                            v_dis = temp_post_decision_state['value'][-1]
                            V_n = (1-self.a) * v_agg + self.a * v_dis

                        else:
                            V_n = temp_post_decision_state['value'][-1]

                        action_values[a] = R_t + self.y * V_n
                        immediate_rewards[a] = R_t

                a_star = max(action_values, key=action_values.get)
                self.policy[current_state_key] = a_star
                # print(f'\nBEST ACTION: ---------> {a_star}')
                ...
                post_decision_state = self.apply_action_to_state(current_state, a_star, t, n)
                post_decision_state_key = self.create_hashable_state_key(post_decision_state)
                aggregate_state_key = self.create_aggregate_state_key(post_decision_state)


                # Update values of aggregated state:
                if not aggregate_state_key in self.agg_states:
                    self.agg_states[aggregate_state_key] = {'count': 1,
                                                            'value': [post_decision_state['value'][-1]],
                                                            'iteration': [n],
                                                            }
                else:
                    v_agg = self.agg_states[aggregate_state_key]['value'][-1]
                    v_dis = post_decision_state['value'][-1]
                    V_n = (1 - self.a) * v_agg + self.a * v_dis

                    self.agg_states[aggregate_state_key]['value'].append(V_n)
                    self.agg_states[aggregate_state_key]['count'] += 1
                    self.agg_states[aggregate_state_key]['iteration'].append(n)

                # print(f'Post decision State:')
                # self.print_state(post_decision_state)

                # update new value of the current state
                v_n_prev = self.states[current_state_key]['value'][-1]
                ...
                self.states[current_state_key]['value'].append(v_n_curr)
            ...
            print(f'Objective value for {self.folder} - {objective_value}')
        ...
        self.objective_value = list(objective_function_values.values())[-1]
...
if __name__ == '__main__':
    ...
    for folder in test_folders:
        print(f"\nTesting benchmark (untrained) for instance {folder}")
        aircraft_data, flight_data, rotations_data, disruptions, recovery_start, recovery_end = read_data(folder)
        m = VFA_ADP(aircraft_data, flight_data, disruptions, recovery_start, recovery_end, folder)
    ...
    print('\nBenchmarked results with untrained model:')
    print(f'\tAverage objective for benchmark: {avg_objective_value}')
...
class VFA_ADP:
    def __init__(self, aircraft_data, flight_data, disruptions, recovery_start, recovery_end):
        ...
        self.periods = {i: start for i, start in enumerate(intervals)}
        self.steps = [i for i in self.periods.keys()]


        self.T = self.steps[-1]
        self.N = 300 # number of iterations
        self.y = 1 # discount factor
        self.a = 0.1 # learning rate or stepsize, fixed
...
######### HELPER FUNCTIONS: #########
    ...
    def num_remaining_flights(self, state):
        n = 0
        current_time = self.periods[state['t']]
        for id in self.aircraft_ids:
            for flight in state[id]['flights']:
                if flight['ADT'] >= current_time:
                    n += 1
        return n
...
################### SOLVE: ###################
    ...
    def simulate_action_to_state(self, current_state, action, t, n):
        ...
        if not next_state_key in self.states:
            ...
            # next_state['value'] = [-1000 * self.num_remaining_flights(next_state)]
    ...
    def apply_action_to_state(self, current_state, action, t, n):
        ...
        if not next_state_key in self.states:
            ...
            # downstream rewards gets closer to zero when less time on horizon
            # next_state['value'] = [self.initial_value * (self.T - next_step)  / self.T]
            # if action == ('swap', '10', 'B777#4'):
            #     print(f'Action {action} applied')
            #     print(f'Initial Value of next state: {-1000 * self.num_conflicts(next_state)}')
    ...
    def plot_values(self, value_dict):
        ...
        plt.plot(iterations, objective_values, linestyle='-', marker= 'x', color='r', label='Expected Value of Initial State')

        # Adding labels and title
        plt.xlabel('Iteration')
        plt.ylabel('Expected Value of initial State')
        plt.title('Expected Value of initial State at Each Iteration')
        plt.grid(True)
        plt.legend()

...
'''TEST ADP ALGORITHM WITH FIRST LEVEL OF AGGREGATION'''
...
class TEST_ADP:
    def __init__(self, aircraft_data, flight_data, disruptions, recovery_start, recovery_end, folder):
        ...
        self.a = 0.25 # learning rate or stepsize, fixed
...
################### SOLVE: ###################
    def solve_with_vfa(self):
        ...
        for t in self.steps[:-1]:
            ...
            self.plot_schedule(S_t_dict, iteration='Test', instance='Test')
            ...
            if S_t in self.policy:
                ...
                for aircraft_id in self.aircraft_ids:
                    ...
                    for x in ac_actions:
                        S_ta_dict = self.simulate_action_to_state(S_t_dict, x, t, n=-1)
                        S_ta = self.create_hashable_state_key(S_ta_dict)
                        S_tx_g = self.G(S_ta_dict)
                        R_t = self.compute_reward(S_t_dict, S_ta_dict, x)
                        ...
                        if S_tx_g in self.agg_states:
                            ...
                            V_n_next = self.agg_states[S_tx_g]['value'][-1]
                        ...
                        # Step 3: Use interpolation/extrapolation if neither exact nor aggregate state is found
                            ...
                            V_n_next = self.interpolate_or_extrapolate(S_tx_g)
                            # print(f'Estimated value function: {V_n_next}')
                        action_values[x] = R_t + self.y * V_n_next
            ...
            if next_state['t'] == self.T:
                self.plot_schedule(next_state, iteration='Test', instance='Test')
...
if __name__ == '__main__':
    ...
    print(f'Results with trained ADP model')

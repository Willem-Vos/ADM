        ...
        for n in range(1, int(self.N)+1):
            ...
            for t in self.steps:
                ...
                if n == self.N:
                    ...
                    self.print_state(current_state_key)
                ...
                for action_set in  valid_action_sets:
                    ...
                    action_values[action_set] = immediate_reward + self.y * downstream_reward  # T EN ACTION ALS KEYS? DOET DE TIJD ERTOE?
                    immediate_rewards[action_set] = immediate_reward


                # Get best action set and following post-decission state
                best_action_set = max(action_values, key=action_values.get)
                ...
                print(f'Best immediate and downstream reward: {action_values[best_action_set]}')
                ...
                if len(current_values) > 1:
                    print(f'new_value = (1 - {self.a}) * {current_value} + {self.a} * {action_values[best_action_set]}')
                    # Approximation function
                    new_value = (1 - self.a) * current_value + self.a * action_values[best_action_set]

                else:
                    print("First Value iteration for state:")
                    new_value = action_values[best_action_set]
                ...
                #
                # # Transition with e-greedy:>>>>>>>>>>>>
                # random_action_set = random.choice(valid_action_sets)
                # action_set = random.choices(population=[best_action_set, random_action_set],
                #                       weights=[1 - self.epsilon, self.epsilon])[0]
                #
                #
                #
                # # Exploration <> Exploitation:
                # if action_set == best_action_set:
                #     realized_immediate_rewards[action_set] = self.compute_total_reward(current_state, action_set)
                #     realized_rewards.append(realized_immediate_rewards[action_set])
                #
                #     # Add the post decisions state to states and update state for the next iteration:
                #     self.states[post_decision_state_key] = post_decision_state
                #     next_state = post_decision_state
                #
                # elif action_set == random_action_set:
                #     realized_immediate_rewards[action_set] = self.compute_total_reward(current_state, action_set)
                #     realized_rewards.append(realized_immediate_rewards[action_set])
                #
                #     # Add the post decisions state to states and update state for the next iteration:
                #     random_decision_state = self.apply_action_set_to_state(current_state, random_action_set, t, n)
                #     random_decision_state_key = self.create_hashable_state_key(random_decision_state)
                #     self.states[random_decision_state_key] = random_decision_state
                #     next_state = random_decision_state
